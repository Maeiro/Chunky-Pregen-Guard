...
function Resolve-DefaultServerRoot {
    ...
    foreach ($base in $candidates) {
        try {
            $direct = [System.IO.Path]::Combine($base, "chunky-pregen-guard.ps1")
            ...
            if (-not [string]::IsNullOrWhiteSpace($parent)) {
                $parentCandidate = [System.IO.Path]::Combine($parent, "chunky-pregen-guard.ps1")
                if ([System.IO.File]::Exists($parentCandidate)) {
                    return $parent
                }
            }
        } catch {}
...
$targetScript = Join-Path $ServerRoot "chunky-pregen-guard.ps1"
if (-not (Test-Path -Path $targetScript)) {
    throw "Nao encontrei chunky-pregen-guard.ps1 em: $targetScript"
}
...
$btnStart.Add_Click({
    try {
        ...
        $diag = Wait-StartupDiagnosis -RootPath $ServerRoot -SupervisorLogRel $ui.LogFile.Text -Since $launchStart -TimeoutSec 55
...
$btnSaveBat.Add_Click({
    try {
        ...
        $args = Get-ArgumentData -ui $ui -ScriptPath ".\chunky-pregen-guard.ps1"
        $parts = @("powershell")
