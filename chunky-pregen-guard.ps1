...
function Get-EmbeddedCoreScript {
    ...
    $replacements = [ordered]@{
        'Arquivo user_jvm_args.txt nao encontrado no diretorio atual.' = 'user_jvm_args.txt not found in current directory.'
        'Arquivo Forge win_args.txt nao encontrado.' = 'Forge win_args.txt file not found.'
        'Solicitando parada:' = 'Requesting stop:'
        'Parada graciosa excedeu timeout de' = 'Graceful stop exceeded timeout of'
        'Forcando encerramento.' = 'Forcing shutdown.'
        'Supervisor iniciado.' = 'Supervisor started.'
        'Politica RAM:' = 'RAM policy:'
        'Comandos de retomada:' = 'Resume commands:'
        'Ja existe um supervisor ativo' = 'There is already an active supervisor'
        'Feche-o antes de iniciar outro.' = 'Close it before starting another.'
        'Ja existe outro supervisor ativo' = 'There is another active supervisor'
        'Ja existe java do servidor rodando' = 'There is already a server java process running'
        'Pare o servidor atual e execute o .bat novamente.' = 'Stop the current server and run the .bat again.'
        'Servidor iniciado (PID=' = 'Server started (PID='
        'Aguardando ' = 'Waiting '
        's para retomada do Chunky.' = 's before resuming Chunky.'
        'Comando enviado apos Servidor iniciado:' = 'Command sent after server start:'
        'Falha ao enviar comando apos start:' = 'Failed to send command after start:'
        'Processo Java encerrou (code=' = 'Java process exited (code='
        'Reiniciando em 10 s.' = 'Restarting in 10 s.'
        'Servidor reiniciado (PID=' = 'Server restarted (PID='
        'Comando enviado apos restart:' = 'Command sent after restart:'
        'Falha ao enviar comando apos restart:' = 'Failed to send command after restart:'
        'Reiniciando servidor em 10 s.' = 'Restarting server in 10 s.'
        'Pico de RAM:' = 'RAM spike:'
        'RAM sustentada:' = 'Sustained RAM:'
        'atual=' = 'current='
        'media=' = 'average='
        'Encerrando supervisor antigo (PID=' = 'Stopping old supervisor (PID='
        'para evitar duplicidade.' = 'to avoid duplication.'
        'Falha ao encerrar supervisor antigo (PID=' = 'Failed to stop old supervisor (PID='
        'Encerrando servidor existente (PID=' = 'Stopping existing server (PID='
        'para assumir supervisao.' = 'to take over supervision.'
        'Falha ao encerrar servidor existente (PID=' = 'Failed to stop existing server (PID='
        'say Reiniciando servidor automaticamente em 20s...' = 'say Automatic restart in 20s...'
    }
    foreach ($kv in $replacements.GetEnumerator()) {
        $scriptText = $scriptText.Replace($kv.Key, $kv.Value)
    }
    return $scriptText
}
